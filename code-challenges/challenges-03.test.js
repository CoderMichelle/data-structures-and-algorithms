...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    resultsArr.push(arr[i] + 2);
  }
  // return resultsArr([3, 4, 6])
  return resultsArr
};
...
const containsAnd = (arr) => {
  let newArr3 = arr.filter(str => str.includes('and'));
...
const notInFirstArray = (forbiddenValues, arr) => {
  const array5 = forbiddenValues.filter(element => {
  ...
  console.log('this is the array, hooray', array5);
  return array5;
};
...
xdescribe('Testing challenge 6', () => {
  ...
  xtest('It should work for non-Snorlax data', () => {
